...
// Initialize datalist with state names and codes
function initializeStateList() {
  const stateList = document.getElementById('stateList');
  
  // Add all state names and codes
  Object.entries(stateMapping).forEach(([name, code]) => {
...
document.getElementById("stateInput").addEventListener("input", (e) => {
  ...
  if (code.length === 2 && stateMapping[code]) {
    e.target.value = `${code} - ${stateMapping[code]}`;
  }
});
...
async function getStateData(stateName) {
  ...
  const t = state.total;
  return {
    name: stateName.charAt(0).toUpperCase() + stateName.slice(1).toLowerCase(),
